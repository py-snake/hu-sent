...
class SentimentAnalysis(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    comment_id = db.Column(db.Text, db.ForeignKey('comments.comment_id'), unique=True)
    sentiment = db.Column(db.String(20))
    confidence = db.Column(db.Float)
    processed_at = db.Column(db.DateTime, server_default=db.func.now())
    comment_rel = db.relationship('Comment', backref=db.backref('sentiment_data', uselist=False))
